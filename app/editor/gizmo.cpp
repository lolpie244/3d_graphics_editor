...
Gizmo::Gizmo(events::Observer& observer) {
    ...
    gizmos_[0]->BindDrag(observer, [this](sf::Event event, glm::vec3 move) {
        move = move * math::axis_to_vector(gizmos_[0]->PressInfo().VertexId) * settings::MOUSE_SENSATIVITY;
        current_model_->Move(move.x, move.y, move.z);
        return true;
    });

    gizmos_[1]->BindDrag(observer, [this](sf::Event event, glm::vec3 move) {
        move = move * math::axis_to_vector(gizmos_[1]->PressInfo().VertexId) * settings::MOUSE_SENSATIVITY;
        auto scale = current_model_->GetScale() + move;
        current_model_->SetScale(scale.x, scale.y, scale.z);
        return true;
    });

    for (auto& gizmo : gizmos_) { gizmo->SetScale(0.5, 0.5, 0.5); }
