...
sf::Vector2f SpriteGuiElement::LeftCorner() const {
    return this->sprite_.getGlobalBounds().getPosition();
}

void SpriteGuiElement::Resize(sf::Vector2<float> size) {
    size_ = size;
    ...
    auto scale = sf::Vector2f(size.x / real_size.x, size.y / real_size.y);
